...
- `Launcher.swift` owns panel lifecycle and input; `Views.swift` owns presentation and the shared panel chrome (`PanelStyle`, `Row`, `GlassPanel`, `keyAction` routing) used by both panels; `Catalog.swift`, `Rank.swift`, and `History.swift` own discovery, matching, and persistence; `Text.swift` owns byte-level string primitives shared by them; `Window.swift` owns frontmost-window tiling; `Clipboard.swift` and `ClipPanel.swift` own pasteboard capture (history, selection grabs, synthesized copy/paste) and the clip panel; `Translate.swift` owns the translate config, the OpenAI-shaped chat client, and the translator; `TransPanel.swift` owns the translate panel and its geometry.
...
- User data lives in `~/Library/Application Support/Launcher/{aliases,usage,hidden}.json` plus `clipboard/` (`index.json` and `blobs/`) and `translate.json` (`base`, `key`, `model`, optional `extra` merged into the request body, `styles` with optional per-style `model` and `extra` overrides); use temporary directories for persistence checks.
